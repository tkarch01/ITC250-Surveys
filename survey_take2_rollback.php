<?php
/**
 * survey_take2_rollback.php shows us what happens when an error occurs and 
 * a transaction must be rolled back
 * 
...
<p><b><font color="red">Warning</font></b> This take on version 2 (rollback) has an error which demonstrates what happens when 
MySQL rolls back the transaction.</p>
<p>The error will effectively 'burn up' ResponseIDs, which will be inserted, then deleted when an error occurs
 during the Choice insertion.</p>
<?php
	...
	{//insert response!
		...
		//insert response
		$sql = sprintf("INSERT into srv_responses(SurveyID,DateAdded) VALUES ('%d',NOW())",$_POST['SurveyID']);
		...
		#ERROR HERE - This was added just to see what happens when we need to rollback during a data insert error:
		mysqli_rollback($iConn);
		die(trigger_error("Purposeful error created here: " . mysqli_error($iConn), E_USER_ERROR));
 
		
		#Will never get below here, since the error added above will always crash		
			 ...
			 {//add choice!
			 	...
			 	{//checkboxes are arrays, and we need to loop through each checked item to insert
				 	while (list ($key,$value) = @each($_POST[$varName])){
					 	$sql = "insert into srv_responses_answers(ResponseID,QuestionID,AnswerID) values($ResponseID,$QuestionID,$value)";
		...
		Survey::responseCount($_POST['SurveyID']);
